...
describe('Delivery Management & State Machine Engine (E2E)', () => {
  ...
  beforeAll(async () => {
    ...
    ownerUser = await prisma.user.create({
      data: {
        username: `del_own_${Date.now()}_${Math.floor(Math.random() * 10000)}`,
    ...
    driverUserA = await prisma.user.create({
      data: {
        username: `del_drv_a_${Date.now()}_${Math.floor(Math.random() * 10000)}`,
    ...
    driverEntityA = await prisma.driver.create({
      data: { userId: driverUserA.id, employeeCode: `DRV-DEL-A-${Date.now()}-${Math.floor(Math.random() * 10000)}`, displayName: 'Driver Del A', phone: driverUserA.phone },
    });
    ...
    driverUserB = await prisma.user.create({
      data: {
        username: `del_drv_b_${Date.now()}_${Math.floor(Math.random() * 10000)}`,

...
describe('Realtime Chat Streaming & ACK Protocol (E2E)', () => {
  ...
  beforeAll(async () => {
    ...
    driverEntityA = await prisma.driver.create({
      data: { userId: driverUserA.id, employeeCode: `DRV-WSC-${Date.now()}`, displayName: 'Driver WS Chat', phone: driverUserA.phone },
    });
